<?php 
    ...
    switch($tabla){
        case 'normal':
            $datosHistoriales = MostrarAuditoriaHistorialAlimenticio($iniciar,3);
            $paginas = ceil(ContarAuditoriaHistorialAlimenticio()/3);
        break;
        case 'busqueda':
            $datosHistoriales = BuscarAuditoriaHistorialAlimenticio($buscar,$iniciar,3);
            $paginas = ceil(ContarAuditoriaHistorialAlimenticioB($buscar)/3);
        break;
    }
...
<body>
    ...
    <div class="row">
        <center><h2>TABLA DE AUDITORIA DE HISTORIALES ALIMENTICIOS</h2></center>
    </div>
            ...
            <td>
                ...
                <?php foreach ($datosHistoriales as $historial ){?>
                    <tr class="tr_x">
                        ...
                        <td class="td_x"><?php echo $historial['audi_hali_cantidad']; ?></td>
                        <td class="td_x"><?php echo $historial['audi_hali_fecha_hora']; ?></td>
        ...
        <tr>
            ...
            <td>
                <nav aria-label="Page navigation example ">
                    <ul class="pagination">
                        <li class="page-item <?php echo $_GET['pagina']<= 1 ? 'disabled' : ''?>"><a class="page-link" href="VistaAuditoriaHistorialesAlimenticios.php?tbl=<?php echo $tabla."&info=".$buscar."&pagina=".$_GET['pagina']-1?>">Anterior</a></li>

                        <?php for($i=0;$i<$paginas;$i++): ?>
                        <li class="page-item <?php echo $_GET['pagina']== $i+1 ? 'active' : ''?>"><a class=" page-link" href="VistaAuditoriaHistorialesAlimenticios.php?tbl=<?php echo $tabla."&info=".$buscar."&pagina=".$i+1?>"> <?php echo $i+1?> </a></li>
                        <?php endfor ?>

                        <li class="page-item <?php echo $_GET['pagina']>= $paginas ? 'disabled' : ''?>"><a class=" page-link" href="VistaAuditoriaHistorialesAlimenticios.php?tbl=<?php echo $tabla."&info=".$buscar."&pagina=".$_GET['pagina']+1?>">Siguiente</a></li>
                    </ul>
                </nav>
            </td>
        </tr>
